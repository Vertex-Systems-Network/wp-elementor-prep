...
- PR #564 / #563 — adds a separate deterministic URL-only core Image asset-proof vector and dedicated observed evidence contract. The real WP `6.8` + Elementor `4.2.4` workflow must import/render that exact candidate, match the rendered Image URL fingerprint to the exact candidate/reference-review identity, and observe a loaded browser image. `ASSET_BOUND_FULL_PASS` remains one controlled localhost-fixture observation only; asset closure, arbitrary external-host reachability, media upload/attachment-ID portability and production compatibility remain false/unclaimed.
...
| P15 Elementor native export + validation | CORE FOUNDATION IN PROGRESS / CONTROLLED TARGET PROOF RETAINED | N/A | `──────────` | Exact proof/profile/candidate binding retained; #552 image mapping, #554 semantics, #556/#558/#560/#562 responsive slices and #564 one controlled URL-only Image observation exist; broader responsive coverage, general asset/reference closure and additional matrix evidence remain pending |
...
2. continue P15 commercial-V1 after #563/#564 by keeping the one controlled URL-only Image observation narrowly scoped, then address the next real asset/reference gap only with separately bound evidence (for example target-managed media/attachment semantics) while adding further responsive controls only when separately documented/bounded; add mappings only through exact source-bound contracts, open target-proof work only for specific new coverage claims, and keep target-compatibility/production/download authority false unless separate evidence explicitly changes it;
