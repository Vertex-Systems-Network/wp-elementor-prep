...
| P15 Elementor native export + validation | CORE FOUNDATION IN PROGRESS / CONTROLLED TARGET PROOF RETAINED | N/A | `──────────` | Exact proof + profile/candidate binding retained; #552 adds exact URL-bound image-review resolution while asset closure evidence, broader semantics/responsive mapping and additional matrix evidence remain pending |
...
The bounded local code path now includes target-neutral IR, deterministic Elementor v0.4 Template JSON candidate generation, read-only selected-Figma-Frame extraction for supported Auto Layout/plain-text facts, and exact source-bound URL-only resolution for `IMAGE_ASSET_EXPORT_REQUIRED` nodes. Resolution never downloads/uploads an asset or invents a target attachment ID; unrelated review nodes are preserved and stale/partial/extra/duplicate/unsafe manifests fail closed. Other REVIEW-bearing extraction/generation still fails closed with no partial candidate, and layer names are not used to invent heading/button semantics.
...
2. continue P15 commercial-V1 after the #551/#552 image-reference resolution train through remaining semantic/responsive mapping plus real reference/asset closure evidence; open additional target-proof work only for specific new coverage claims, and keep target-compatibility/production/download authority false unless separate evidence explicitly changes it;
