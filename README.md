...
- alias-safe output writes: the real output parent is resolved, existing symlink/non-regular targets are rejected, hardlink identity against the files actually read is rejected where stable identity is available, and output is staged in a unique same-directory regular temp file before atomic rename.
