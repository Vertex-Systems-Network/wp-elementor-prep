...
- PR #568 / #567 — adds a separate target-managed media content-integrity evidence contract. The controlled workflow hashes the exact served PNG fixture and the imported WordPress attachment's original file, then requires exact SHA-256 equality plus attachment post type, `image/png` MIME and positive image dimensions for `TARGET_MANAGED_CONTENT_INTEGRITY_PASS`. Filesystem paths and numeric attachment IDs are not emitted, and the pass remains non-portability/non-closure evidence.
...
| P15 Elementor native export + validation | CORE FOUNDATION IN PROGRESS / CONTROLLED TARGET PROOF RETAINED | N/A | `──────────` | Exact proof/profile/candidate binding retained; bounded semantic/responsive slices, #564 managed-media observation, #566 observed-reference binding and #568 exact imported-file content-integrity evidence exist; durable portability, internal closure decision, broader responsive coverage and additional matrix evidence remain pending |
...
2. continue P15 commercial-V1 after #567/#568 by keeping observed proof, exact imported-file integrity and any future internal closure decision as separate states; next target durable target-managed media portability/decision evidence, add responsive controls only through separately documented exact contracts, and keep target-compatibility/production/download authority false unless separate evidence explicitly changes it;
