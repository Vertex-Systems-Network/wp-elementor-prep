...
import {
  ...
  P15_ELEMENTOR_FIRST_PROOF_VECTOR_VERSION,
  buildP15ElementorFirstProofVector,
} from '../targets/elementor/first-proof-vector';

const DEFAULT_OUT_DIR = `dist-p15/${P15_ELEMENTOR_FIRST_PROOF_VECTOR_VERSION}`;

function fail(message: string): never {
  process.stderr.write(`P15_ELEMENTOR_FIRST_PROOF_VECTOR_FAILED: ${message}\n`);
  process.exit(2);
}
...
async function verifyExistingVector(
  ...
  const expectedNames = [...P15_ELEMENTOR_FIRST_PROOF_VECTOR_FILENAMES].sort();
