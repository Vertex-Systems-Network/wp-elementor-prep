...
async function verifyExistingVector(
  ...
  if (actualNames.length !== expectedNames.length
    ...
    fail('Existing asset vector directory contains missing or unexpected files; refusing to overwrite it.');
  }
  ...
  for (const filename of P15_ELEMENTOR_ASSET_PROOF_VECTOR_FILENAMES) {
    ...
    if (!actual.equals(expectedBytes)) {
      fail(`Existing asset vector drift detected in ${filename}; refusing to overwrite it.`);
    }
  }
}
...
async function writeNewVector(
  ...
  try {
    ...
    if (await pathExists(outDir)) {
      throw new Error('Asset vector output appeared during generation; refusing to overwrite it.');
    }
...
async function main(): Promise<void> {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    assetReference: vector.manifest.assetReference,
