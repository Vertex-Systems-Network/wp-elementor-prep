...
export type P16StableContentObservation = {
...
/**
 * Stream-hash one current bounded P16 operator input through an opened file handle.
 * Path/canonical identity and file metadata are checked around the hash so replacement
 * or retarget races fail closed before the caller compares the digest to retained evidence.
 */
export async function observeP16StableInputContent(
  path: string,
): Promise<P16StableContentObservation | null> {
