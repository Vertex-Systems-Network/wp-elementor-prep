...
import { validateElementorAssetTargetProofEvidence } from '../targets/elementor/asset-target-proof-evidence';
...
const DEFAULT_OUT = 'dist-p15/elementor-asset-target-proof-intake.json';

function fail(message: string, exitCode = 2): never {
  process.stderr.write(`P15_ELEMENTOR_ASSET_TARGET_PROOF_INTAKE_FAILED: ${message}\n`);
...
const report = {
  schemaVersion: 1,
  gate: 'p15-elementor-asset-target-proof-intake-v1',
  ...
  assetReferenceBindingMatches: validation.assetReferenceBindingMatches,
  declaredObservedEnvironmentMatches: validation.declaredObservedEnvironmentMatches,
  renderedReferenceMatches: validation.renderedReferenceMatches,
  candidateIdentity: validation.candidateIdentity,
  targetProfileFingerprint: validation.targetProfileFingerprint,
  referenceReviewIdentityDigest: validation.referenceReviewIdentityDigest,
  expectedAssetReference: validation.expectedAssetReference,
...
process.stdout.write(`${JSON.stringify({
  ...
  assetReferenceBindingMatches: report.assetReferenceBindingMatches,
  declaredObservedEnvironmentMatches: report.declaredObservedEnvironmentMatches,
  renderedReferenceMatches: report.renderedReferenceMatches,
  assetReferenceClosureClaim: false,
}, null, 2)}\n`);
