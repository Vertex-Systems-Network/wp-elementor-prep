...
function snapshotEvidence(value: Record<string, unknown>): ElementorTargetEnvironmentEvidenceV1 | null {
  ...
  return {
    ...
    database: { engine: database.engine, version: database.version },
    wordpressMemoryLimitMb: value.wordpressMemoryLimitMb,
    browser: { family: browser.family, version: browser.version },
...
export function validateElementorTargetEnvironmentEvidence(value: unknown): ElementorTargetEnvironmentValidationResult {
  ...
  if (!isRecord(value)) {
    ...
    if (!parseNumericVersion(value.wordpressVersion)
      || !parseNumericVersion(value.elementorVersion)
      || !parseNumericVersion(value.phpVersion)) {
      issues.push({
  ...
  if (issues.length === 0 && environment) {
    const wordpressVersion = parseNumericVersion(environment.wordpressVersion)!;
    const phpVersion = parseNumericVersion(environment.phpVersion)!;
    ...
    requirementChecks.wordpressMinimum = versionAtLeast(wordpressVersion, MIN_WORDPRESS);
    if (!requirementChecks.wordpressMinimum) failures.push('P15_TARGET_ENVIRONMENT_WORDPRESS_BELOW_MINIMUM');

    requirementChecks.phpMinimum = versionAtLeast(phpVersion, MIN_PHP);
    if (!requirementChecks.phpMinimum) failures.push('P15_TARGET_ENVIRONMENT_PHP_BELOW_MINIMUM');

    const databaseEngine = environment.database.engine;
    if (databaseEngine === 'MYSQL') {
      requirementChecks.databaseSupported = true;
      requirementChecks.databaseMinimum = versionAtLeast(databaseVersion, MIN_MYSQL);
      ...
      requirementChecks.databaseSupported = true;
      requirementChecks.databaseMinimum = versionAtLeast(databaseVersion, MIN_MARIADB);
      if (!requirementChecks.databaseMinimum) failures.push('P15_TARGET_ENVIRONMENT_DATABASE_BELOW_MINIMUM');
    } else {
    ...
    const browserFamily = environment.browser.family;
    if (browserFamily === 'OTHER') {
      failures.push('P15_TARGET_ENVIRONMENT_BROWSER_UNSUPPORTED');
    } else {
      requirementChecks.browserSupported = true;
      requirementChecks.browserMinimum = versionAtLeast(browserVersion, BROWSER_MINIMUMS[browserFamily]);
  ...
  const classification: ElementorTargetEnvironmentClassification = !valid
    ? 'REJECTED'
    : failures.length > 0
      ? 'NOT_QUALIFIED'
...
export function buildElementorTargetEnvironmentEvidence(
  input: Omit<ElementorTargetEnvironmentEvidenceV1,
    'schemaVersion' | 'evidenceVersion' | 'source' | 'acceptanceAuthority' | 'targetCompatibilityClaim' | 'productionAcceptance' | 'internalReviewRequired'>,
