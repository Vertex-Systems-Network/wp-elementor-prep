...
const ISSUE_CODES: readonly P15ElementorButtonHoverBackgroundColorIssueCode[] = [
  'P15_BUTTON_HOVER_BACKGROUND_COLOR_SOURCE_IR_INVALID',
  'P15_BUTTON_HOVER_BACKGROUND_COLOR_UPSTREAM_GENERATION_NOT_READY',
  'P15_BUTTON_HOVER_BACKGROUND_COLOR_MANIFEST_NOT_OBJECT',
  'P15_BUTTON_HOVER_BACKGROUND_COLOR_MANIFEST_FIELDS_INVALID',
...
function validColor(value: unknown): value is P15ElementorButtonHoverBackgroundColorValue {
...
function baseResult(
  ...
  return {
    ...
    sourceButtonCount,
    resolvedButtonCount: resolvedHoverBackgrounds.length,
    resolvedHoverBackgrounds: resolvedHoverBackgrounds.map((entry) => ({ ...entry })),
...
export function resolveP15ElementorButtonHoverBackgroundColors(
  ...
  if (!validation.valid) {
    return baseResult(
      ...
      validation.issues.map((issue) => ({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_SOURCE_IR_INVALID' as const,
        path: issue.path,
  ...
  if (baseGeneration.status !== 'GENERATED_LOCAL_CANDIDATE'
    ...
    return baseResult(
      ...
      [],
      [{
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_UPSTREAM_GENERATION_NOT_READY',
        path: '$source',
        message: 'Button hover background color resolution requires an existing review-free generated local candidate.',
      }],
      null,
      null,
    );
  ...
  const issues: P15ElementorButtonHoverBackgroundColorIssueV1[] = [];
  const resolutions = new Map<string, P15ElementorButtonHoverBackgroundColorEntryV1>();

  if (!isRecord(manifestValue)) {
    issues.push({
      code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_MANIFEST_NOT_OBJECT',
      path: '$manifest',
      message: 'Button hover background color manifest must be an object.',
    });
  } else {
    if (!exactKeys(manifestValue, MANIFEST_KEYS)) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_MANIFEST_FIELDS_INVALID',
        path: '$manifest',
        message: 'Button hover background color manifest contains unknown or missing fields.',
      });
    }

    if (manifestValue.schemaVersion !== 1
      || manifestValue.manifestVersion !== P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_MANIFEST_VERSION) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_MANIFEST_VERSION_INVALID',
        path: '$manifest.manifestVersion',
        message: 'Button hover background color manifest schema/version is unsupported.',
      });
    }

    if (!validFingerprint(manifestValue.sourceIrFingerprint)) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_SOURCE_FINGERPRINT_INVALID',
      ...
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_SOURCE_FINGERPRINT_MISMATCH',
        path: '$manifest.sourceIrFingerprint',
    ...
    if (!validFingerprint(manifestValue.baseCandidateIdentityDigest)) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_BASE_CANDIDATE_IDENTITY_INVALID',
      ...
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_BASE_CANDIDATE_IDENTITY_MISMATCH',
        path: '$manifest.baseCandidateIdentityDigest',
    ...
    if (manifestValue.colorInferencePerformed !== false
      ...
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_AUTHORITY_FLAGS_INVALID',
        path: '$manifest',
        message: 'Button hover background color resolution cannot grant color/responsive inference, mutation, network, closure, compatibility, production or download authority.',
      });
    }

    if (!Array.isArray(manifestValue.buttons)
      || manifestValue.buttons.length > P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_MAX_ENTRIES) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_ENTRIES_INVALID',
        path: '$manifest.buttons',
        message: `buttons must be an array of at most ${P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_MAX_ENTRIES} entries.`,
      });
    } else {
      for (let index = 0; index < manifestValue.buttons.length; index += 1) {
        ...
        if (!isRecord(raw)
          ...
          issues.push({
            code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_ENTRY_INVALID',
            path,
            message: 'Each Button hover background color entry may contain only sourceNodeId and color.',
          });
          continue;
        }
        ...
        if (resolutions.has(sourceNodeId)) {
          issues.push({
            code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_DUPLICATE_SOURCE_ID',
            path: `${path}.sourceNodeId`,
            message: 'Button hover background color sourceNodeId must be unique.',
          });
          continue;
        }
        ...
        if (!sourceButtons.has(sourceNodeId)) {
          issues.push({
            code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_SOURCE_NOT_BUTTON',
            path: `${path}.sourceNodeId`,
            message: 'Button hover background color sourceNodeId must identify an existing neutral Button node.',
          });
          continue;
        }
        ...
        if (!validColor(raw.color)) {
          issues.push({
            code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_VALUE_INVALID',
            path: `${path}.color`,
  ...
  if (binding.issues.length > 0) {
    return baseResult(
      ...
      binding.issues.map((issue) => ({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_GENERATOR_BINDING_MISMATCH' as const,
        path: issue.path,
        message: issue.message,
      })),
      null,
  ...
  for (const [sourceNodeId, resolution] of resolutions) {
    ...
    if (!target || !isRecord(target.settings)) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_GENERATOR_BINDING_MISMATCH',
        path: '$.content',
    ...
    const hasBackgroundType = Object.prototype.hasOwnProperty.call(
      target.settings,
    ...
    if (hasBackgroundType || hasBackgroundColor) {
      issues.push({
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_EXISTING_OVERRIDE_CONFLICT',
        path: `$source.${sourceNodeId}`,
        message: 'Generated base candidate already contains a Button hover background override.',
      });
      continue;
    }

    target.settings[P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_EVIDENCE.backgroundTypeSettingKey]
      = P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_EVIDENCE.acceptedBackgroundType;
    target.settings[P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_EVIDENCE.backgroundColorSettingKey]
      = resolution.color;
  }
  ...
  if (candidate.status !== 'READY_FOR_TARGET_IMPORT_VALIDATION'
    ...
    return baseResult(
      ...
      [],
      [{
        code: 'P15_BUTTON_HOVER_BACKGROUND_COLOR_RESOLVED_CANDIDATE_INVALID',
        path: '$resolvedCandidate',
        message: 'Button hover background color output did not rebuild into a canonical ready Elementor candidate.',
      }],
      null,
      null,
    );
  ...
  const resolvedHoverBackgrounds = [...resolutions.values()]
    .map((entry) => ({ ...entry }))
...
function validIssue(issue: P15ElementorButtonHoverBackgroundColorIssueV1): boolean {
  return isRecord(issue)
    && typeof issue.code === 'string'
    && ISSUE_CODES.includes(issue.code as P15ElementorButtonHoverBackgroundColorIssueCode)
    && typeof issue.path === 'string'
    && issue.path.length > 0
    && issue.path.length <= 1024;
}

function validSummaryEntry(entry: P15ElementorButtonHoverBackgroundColorSummaryEntryV1): boolean {
...
export function serializeP15ElementorButtonHoverBackgroundColorSummary(
  ...
  const validStatus = result.status === 'BLOCKED_INVALID_SOURCE_IR'
    ...
    || result.status === 'NO_BUTTON_HOVER_BACKGROUND_COLOR_OVERRIDES'
    || result.status === 'BUTTON_HOVER_BACKGROUND_COLORS_RESOLVED';
  ...
  const validCounts = Number.isSafeInteger(result.sourceButtonCount)
    ...
    && result.resolvedButtonCount === result.resolvedHoverBackgrounds.length;

  const uniqueIds = new Set(result.resolvedHoverBackgrounds.map((entry) => entry.sourceNodeId)).size
    === result.resolvedHoverBackgrounds.length;
  ...
  const resolvedDigestRequired = result.status === 'NO_BUTTON_HOVER_BACKGROUND_COLOR_OVERRIDES'
    || result.status === 'BUTTON_HOVER_BACKGROUND_COLORS_RESOLVED';
  ...
  const statusShapeValid = result.status === 'BUTTON_HOVER_BACKGROUND_COLORS_RESOLVED'
    ? result.resolvedButtonCount > 0
  ...
  if (!validStatus
    ...
    || !validResolvedDigest
    || !statusShapeValid
    || !result.resolvedHoverBackgrounds.every(validSummaryEntry)
    ...
    throw new Error('Invalid or authority-inflated P15 button-hover-background-color result.');
  }
  ...
  return `${JSON.stringify({
    ...
    resolvedHoverBackgrounds: result.resolvedHoverBackgrounds.map((entry) => ({ ...entry })),
    issues: result.issues.map((issue) => ({ code: issue.code, path: issue.path })),
    evidence: P15_ELEMENTOR_BUTTON_HOVER_BACKGROUND_COLOR_EVIDENCE,
    colorInferencePerformed: false,
    responsiveInferencePerformed: false,
    figmaMutation: false,
    networkAccess: false,
