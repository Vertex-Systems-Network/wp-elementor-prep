...
const ISSUE_CODES: readonly P15ElementorResponsiveHoverBorderRadiusIssueCode[] = [
  'P15_RESPONSIVE_HOVER_BORDER_RADIUS_SOURCE_IR_INVALID',
  'P15_RESPONSIVE_HOVER_BORDER_RADIUS_UPSTREAM_GENERATION_NOT_READY',
  'P15_RESPONSIVE_HOVER_BORDER_RADIUS_MANIFEST_NOT_OBJECT',
  'P15_RESPONSIVE_HOVER_BORDER_RADIUS_MANIFEST_FIELDS_INVALID',
...
function cloneSummaryEntry(
  entry: P15ElementorResponsiveHoverBorderRadiusSummaryEntryV1,
): P15ElementorResponsiveHoverBorderRadiusSummaryEntryV1 {
  return {
...
function baseResult(
  ...
  return {
    ...
    resolvedContainerCount: resolvedHoverBorderRadii.length,
    resolvedHoverBorderRadii: resolvedHoverBorderRadii.map(cloneSummaryEntry),
...
export function resolveP15ElementorResponsiveContainerHoverBorderRadius(
  ...
  if (!validation.valid) {
    return baseResult(
      ...
      validation.issues.map((issue) => ({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_SOURCE_IR_INVALID' as const,
        path: issue.path,
  ...
  if (baseGeneration.status !== 'GENERATED_LOCAL_CANDIDATE'
    ...
    return baseResult(
      ...
      [],
      [{
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_UPSTREAM_GENERATION_NOT_READY',
        path: '$source',
        message: 'Responsive hover border radius resolution requires an existing review-free generated local candidate.',
      }],
      null,
      null,
    );
  ...
  const issues: P15ElementorResponsiveHoverBorderRadiusIssueV1[] = [];
  const resolutions = new Map<string, P15ElementorResponsiveHoverBorderRadiusEntryV1>();

  if (!isRecord(manifestValue)) {
    issues.push({
      code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_MANIFEST_NOT_OBJECT',
      path: '$manifest',
      message: 'Responsive hover border radius manifest must be an object.',
    });
  } else {
    if (!exactKeys(manifestValue, MANIFEST_KEYS)) {
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_MANIFEST_FIELDS_INVALID',
        path: '$manifest',
        message: 'Responsive hover border radius manifest contains unknown or missing fields.',
      });
    }
    if (manifestValue.schemaVersion !== 1
      || manifestValue.manifestVersion !== P15_ELEMENTOR_RESPONSIVE_HOVER_BORDER_RADIUS_MANIFEST_VERSION) {
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_MANIFEST_VERSION_INVALID',
        path: '$manifest.manifestVersion',
        message: 'Responsive hover border radius manifest schema/version is unsupported.',
      });
    }
    if (!validFingerprint(manifestValue.sourceIrFingerprint)) {
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_SOURCE_FINGERPRINT_INVALID',
      ...
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_SOURCE_FINGERPRINT_MISMATCH',
        path: '$manifest.sourceIrFingerprint',
    ...
    if (!validFingerprint(manifestValue.baseCandidateIdentityDigest)) {
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_BASE_CANDIDATE_IDENTITY_INVALID',
      ...
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_BASE_CANDIDATE_IDENTITY_MISMATCH',
        path: '$manifest.baseCandidateIdentityDigest',
    ...
    if (manifestValue.responsiveInferencePerformed !== false
      ...
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_AUTHORITY_FLAGS_INVALID',
        path: '$manifest',
        message: 'Responsive hover border radius resolution cannot grant inference/mutation/network/closure/compatibility/production/download authority.',
      });
    }

    if (!Array.isArray(manifestValue.containers)
      || manifestValue.containers.length > P15_ELEMENTOR_RESPONSIVE_HOVER_BORDER_RADIUS_MAX_ENTRIES) {
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_ENTRIES_INVALID',
        path: '$manifest.containers',
        message: `containers must be an array of at most ${P15_ELEMENTOR_RESPONSIVE_HOVER_BORDER_RADIUS_MAX_ENTRIES} entries.`,
      });
    } else {
      for (let index = 0; index < manifestValue.containers.length; index += 1) {
        ...
        if (!isRecord(raw)
          ...
          issues.push({
            code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_ENTRY_INVALID',
            path,
            message: 'Each responsive hover border radius entry may contain only sourceNodeId plus tablet/mobile border-radius values.',
          });
          continue;
        }
        ...
        if (resolutions.has(sourceNodeId)) {
          issues.push({
            code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_DUPLICATE_SOURCE_ID',
            path: `${path}.sourceNodeId`,
            message: 'Responsive hover border radius sourceNodeId must be unique.',
          });
          continue;
        }
        if (!sourceContainers.has(sourceNodeId)) {
          issues.push({
            code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_SOURCE_NOT_CONTAINER',
            path: `${path}.sourceNodeId`,
            message: 'Responsive hover border radius sourceNodeId must identify an existing neutral container node.',
          });
          continue;
        }
        ...
        if (!tabletProvided && !mobileProvided) {
          issues.push({
            code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_OVERRIDE_REQUIRED',
            path,
            message: 'Each responsive hover border radius entry must explicitly provide tabletCornerRadiusPx and/or mobileCornerRadiusPx.',
          });
          continue;
        }
        ...
        if ((tabletProvided && !validCornerRadiusPx(raw.tabletCornerRadiusPx))
          ...
          issues.push({
            code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_VALUE_INVALID',
            path,
            message: `Responsive hover border radius must be an integer px value between 0 and ${P15_NEUTRAL_EXPORT_MAX_RADIUS_PX}.`,
          });
          continue;
        }
  ...
  if (binding.issues.length > 0) {
    return baseResult(
      ...
      binding.issues.map((issue) => ({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_GENERATOR_BINDING_MISMATCH' as const,
        path: issue.path,
        message: issue.message,
      })),
      null,
  ...
  for (const [sourceNodeId, resolution] of resolutions) {
    ...
    if (!target || !isRecord(target.settings)) {
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_GENERATOR_BINDING_MISMATCH',
        path: '$.content',
    ...
    if (resolution.tabletCornerRadiusPx !== undefined
      ...
      issues.push({
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_EXISTING_OVERRIDE_CONFLICT',
    ...
    if (resolution.tabletCornerRadiusPx !== undefined) {
      target.settings[P15_ELEMENTOR_RESPONSIVE_HOVER_BORDER_RADIUS_EVIDENCE.tabletSettingKey] =
        elementorDimensions(resolution.tabletCornerRadiusPx);
    }
    if (resolution.mobileCornerRadiusPx !== undefined) {
      target.settings[P15_ELEMENTOR_RESPONSIVE_HOVER_BORDER_RADIUS_EVIDENCE.mobileSettingKey] =
  ...
  if (candidate.status !== 'READY_FOR_TARGET_IMPORT_VALIDATION'
    ...
    return baseResult(
      ...
      [],
      [{
        code: 'P15_RESPONSIVE_HOVER_BORDER_RADIUS_RESOLVED_CANDIDATE_INVALID',
        path: '$resolvedCandidate',
        message: 'Responsive hover border radius output did not rebuild into a canonical ready Elementor candidate.',
      }],
      null,
      null,
    );
  ...
  const resolvedHoverBorderRadii = [...resolutions.values()]
    .map((entry) => ({
      sourceNodeId: entry.sourceNodeId,
...
function validIssue(issue: P15ElementorResponsiveHoverBorderRadiusIssueV1): boolean {
  return isRecord(issue)
    && typeof issue.code === 'string'
    && ISSUE_CODES.includes(issue.code as P15ElementorResponsiveHoverBorderRadiusIssueCode)
    && typeof issue.path === 'string'
    && issue.path.length > 0
    && issue.path.length <= 1024;
}

function validSummaryEntry(entry: P15ElementorResponsiveHoverBorderRadiusSummaryEntryV1): boolean {
...
export function serializeP15ElementorResponsiveHoverBorderRadiusSummary(
  ...
  const validStatus = result.status === 'BLOCKED_INVALID_SOURCE_IR'
    ...
    || result.status === 'NO_RESPONSIVE_HOVER_BORDER_RADIUS_OVERRIDES'
    || result.status === 'RESPONSIVE_HOVER_BORDER_RADIUS_RESOLVED';
  const validCounts = Number.isSafeInteger(result.sourceContainerCount)
    ...
    && result.resolvedContainerCount === result.resolvedHoverBorderRadii.length;
  const uniqueIds = new Set(result.resolvedHoverBorderRadii.map((entry) => entry.sourceNodeId)).size
    === result.resolvedHoverBorderRadii.length;
  ...
  const resolvedDigestRequired = result.status === 'NO_RESPONSIVE_HOVER_BORDER_RADIUS_OVERRIDES'
    || result.status === 'RESPONSIVE_HOVER_BORDER_RADIUS_RESOLVED';
  ...
  const statusShapeValid = result.status === 'RESPONSIVE_HOVER_BORDER_RADIUS_RESOLVED'
    ? result.resolvedContainerCount > 0
  ...
  if (!validStatus
    ...
    || !validResolvedDigest
    || !statusShapeValid
    || !result.resolvedHoverBorderRadii.every(validSummaryEntry)
    ...
    throw new Error('Invalid or authority-inflated P15 responsive-hover-border-radius result.');
  }
  ...
  return `${JSON.stringify({
    ...
    resolvedHoverBorderRadii: result.resolvedHoverBorderRadii.map(cloneSummaryEntry),
    issues: result.issues.map((issue) => ({ code: issue.code, path: issue.path })),
    evidence: P15_ELEMENTOR_RESPONSIVE_HOVER_BORDER_RADIUS_EVIDENCE,
    responsiveInferencePerformed: false,
    figmaMutation: false,
    networkAccess: false,
    responsiveClosureClaim: false,
