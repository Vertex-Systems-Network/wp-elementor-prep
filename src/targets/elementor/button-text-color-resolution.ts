...
const ISSUE_CODES: readonly P15ElementorButtonTextColorIssueCode[] = [
  'P15_BUTTON_TEXT_COLOR_SOURCE_IR_INVALID',
  'P15_BUTTON_TEXT_COLOR_UPSTREAM_GENERATION_NOT_READY',
  'P15_BUTTON_TEXT_COLOR_MANIFEST_NOT_OBJECT',
  'P15_BUTTON_TEXT_COLOR_MANIFEST_FIELDS_INVALID',
...
function validColor(value: unknown): value is P15ElementorButtonTextColorValue {
...
function baseResult(
  ...
  return {
    ...
    sourceButtonCount,
    resolvedButtonCount: resolvedColors.length,
...
export function resolveP15ElementorButtonTextColors(
  ...
  if (!validation.valid) {
    return baseResult(
      ...
      validation.issues.map((issue) => ({
        code: 'P15_BUTTON_TEXT_COLOR_SOURCE_IR_INVALID' as const,
        path: issue.path,
  ...
  if (baseGeneration.status !== 'GENERATED_LOCAL_CANDIDATE'
    ...
    return baseResult(
      ...
      [],
      [{
        code: 'P15_BUTTON_TEXT_COLOR_UPSTREAM_GENERATION_NOT_READY',
        path: '$source',
        message: 'Button text color resolution requires an existing review-free generated local candidate.',
      }],
      null,
      null,
    );
  ...
  const issues: P15ElementorButtonTextColorIssueV1[] = [];
  const resolutions = new Map<string, P15ElementorButtonTextColorEntryV1>();

  if (!isRecord(manifestValue)) {
    issues.push({
      code: 'P15_BUTTON_TEXT_COLOR_MANIFEST_NOT_OBJECT',
      path: '$manifest',
      message: 'Button text color manifest must be an object.',
    });
  } else {
    if (!exactKeys(manifestValue, MANIFEST_KEYS)) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_MANIFEST_FIELDS_INVALID',
        path: '$manifest',
        message: 'Button text color manifest contains unknown or missing fields.',
      });
    }

    if (manifestValue.schemaVersion !== 1
      || manifestValue.manifestVersion !== P15_ELEMENTOR_BUTTON_TEXT_COLOR_MANIFEST_VERSION) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_MANIFEST_VERSION_INVALID',
        path: '$manifest.manifestVersion',
        message: 'Button text color manifest schema/version is unsupported.',
      });
    }

    if (!validFingerprint(manifestValue.sourceIrFingerprint)) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_SOURCE_FINGERPRINT_INVALID',
      ...
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_SOURCE_FINGERPRINT_MISMATCH',
        path: '$manifest.sourceIrFingerprint',
    ...
    if (!validFingerprint(manifestValue.baseCandidateIdentityDigest)) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_BASE_CANDIDATE_IDENTITY_INVALID',
      ...
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_BASE_CANDIDATE_IDENTITY_MISMATCH',
        path: '$manifest.baseCandidateIdentityDigest',
    ...
    if (manifestValue.colorInferencePerformed !== false
      ...
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_AUTHORITY_FLAGS_INVALID',
        path: '$manifest',
        message: 'Button text color resolution cannot grant color/responsive inference, mutation, network, closure, compatibility, production or download authority.',
      });
    }

    if (!Array.isArray(manifestValue.buttons)
      || manifestValue.buttons.length > P15_ELEMENTOR_BUTTON_TEXT_COLOR_MAX_ENTRIES) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_ENTRIES_INVALID',
        path: '$manifest.buttons',
        message: `buttons must be an array of at most ${P15_ELEMENTOR_BUTTON_TEXT_COLOR_MAX_ENTRIES} entries.`,
      });
    } else {
      for (let index = 0; index < manifestValue.buttons.length; index += 1) {
        ...
        if (!isRecord(raw)
          ...
          issues.push({
            code: 'P15_BUTTON_TEXT_COLOR_ENTRY_INVALID',
            path,
            message: 'Each Button text color entry may contain only sourceNodeId and color.',
          });
          continue;
        }
        ...
        if (resolutions.has(sourceNodeId)) {
          issues.push({
            code: 'P15_BUTTON_TEXT_COLOR_DUPLICATE_SOURCE_ID',
            path: `${path}.sourceNodeId`,
            message: 'Button text color sourceNodeId must be unique.',
          });
          continue;
        }
        ...
        if (!sourceButtons.has(sourceNodeId)) {
          issues.push({
            code: 'P15_BUTTON_TEXT_COLOR_SOURCE_NOT_BUTTON',
            path: `${path}.sourceNodeId`,
            message: 'Button text color sourceNodeId must identify an existing neutral Button node.',
          });
          continue;
        }
        ...
        if (!validColor(raw.color)) {
          issues.push({
            code: 'P15_BUTTON_TEXT_COLOR_VALUE_INVALID',
            path: `${path}.color`,
  ...
  if (binding.issues.length > 0) {
    return baseResult(
      ...
      binding.issues.map((issue) => ({
        code: 'P15_BUTTON_TEXT_COLOR_GENERATOR_BINDING_MISMATCH' as const,
        path: issue.path,
        message: issue.message,
      })),
      null,
  ...
  for (const [sourceNodeId, resolution] of resolutions) {
    ...
    if (!target || !isRecord(target.settings)) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_GENERATOR_BINDING_MISMATCH',
        path: '$.content',
    ...
    if (Object.prototype.hasOwnProperty.call(
      target.settings,
      P15_ELEMENTOR_BUTTON_TEXT_COLOR_EVIDENCE.settingKey,
    )) {
      issues.push({
        code: 'P15_BUTTON_TEXT_COLOR_EXISTING_OVERRIDE_CONFLICT',
  ...
  if (candidate.status !== 'READY_FOR_TARGET_IMPORT_VALIDATION'
    ...
    return baseResult(
      ...
      [],
      [{
        code: 'P15_BUTTON_TEXT_COLOR_RESOLVED_CANDIDATE_INVALID',
        path: '$resolvedCandidate',
        message: 'Button text color output did not rebuild into a canonical ready Elementor candidate.',
      }],
      null,
      null,
    );
...
function validIssue(issue: P15ElementorButtonTextColorIssueV1): boolean {
  return isRecord(issue)
    && typeof issue.code === 'string'
    && ISSUE_CODES.includes(issue.code as P15ElementorButtonTextColorIssueCode)
    && typeof issue.path === 'string'
    && issue.path.length > 0
    && issue.path.length <= 1024;
}

function validSummaryEntry(entry: P15ElementorButtonTextColorSummaryEntryV1): boolean {
...
export function serializeP15ElementorButtonTextColorSummary(
  ...
  const validStatus = result.status === 'BLOCKED_INVALID_SOURCE_IR'
    ...
    || result.status === 'NO_BUTTON_TEXT_COLOR_OVERRIDES'
    || result.status === 'BUTTON_TEXT_COLORS_RESOLVED';
  ...
  const validCounts = Number.isSafeInteger(result.sourceButtonCount)
    ...
    && result.resolvedButtonCount === result.resolvedColors.length;
  ...
  const resolvedDigestRequired = result.status === 'NO_BUTTON_TEXT_COLOR_OVERRIDES'
    || result.status === 'BUTTON_TEXT_COLORS_RESOLVED';
  ...
  const statusShapeValid = result.status === 'BUTTON_TEXT_COLORS_RESOLVED'
    ? result.resolvedButtonCount > 0
  ...
  return `${JSON.stringify({
    ...
    evidence: P15_ELEMENTOR_BUTTON_TEXT_COLOR_EVIDENCE,
    colorInferencePerformed: false,
    responsiveInferencePerformed: false,
    figmaMutation: false,
    networkAccess: false,
