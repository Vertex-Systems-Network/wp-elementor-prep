 ...
 * It deliberately does not create P14PreparationConfirmationV1 and cannot authorize mutation.
 */
export function buildP14ProposedChangeReviewManifest(
  ...
  const actionsById = new Map(plan.actions.map((action) => [action.actionId, action]));
  const actions: P14ProposedChangeReviewActionV1[] = [];

  for (const actionId of plan.eligibleActionIds) {
