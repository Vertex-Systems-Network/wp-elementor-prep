...
/**
 * Builds a compact serializable closure-evidence record from the exact compiled-runtime self-test.
 * Acceptance is always recomputed from the captured result and current compiled build identity so
 * callers cannot supply a contradictory PASS. The bundle contains no PNG bytes, Figma nodes or
 * mutation candidates.
 */
