...
`selected Figma Frame -> deterministic extraction -> exact source-bound semantic/image-reference resolution where supplied -> exact bounded responsive direction/gap overrides where supplied -> mapping readiness -> fresh candidate -> local artifact validation/download -> exact declared TargetProfile -> exact retained profile/candidate reference binding -> real reference/asset closure + remaining responsive work -> additional controlled target observations where evidence is required -> bounded handoff`
