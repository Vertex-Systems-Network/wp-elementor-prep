...
`selected Figma Frame -> deterministic extraction -> exact source-bound semantic/image-reference resolution where supplied -> mapping readiness -> fresh candidate -> local artifact validation/download -> exact declared TargetProfile -> exact retained profile/candidate reference binding -> real reference/asset closure + responsive work -> additional controlled target observations where evidence is required -> bounded handoff`
