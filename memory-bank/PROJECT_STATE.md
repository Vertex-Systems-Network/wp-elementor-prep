...
- PR #568 / #567 adds a separate target-managed media content-integrity proof layer. The disposable workflow hashes the exact controlled fixture bytes and the imported WordPress attachment original file, then requires exact SHA-256 equality, attachment post type, `image/png` MIME and positive dimensions for `TARGET_MANAGED_CONTENT_INTEGRITY_PASS`; no file path, numeric attachment ID or portability authority is retained.
