...
The retained foundation includes normalized Gutenberg candidate/capability/profile contracts, exact candidate identity, external receipt/offline revalidation, sanitized review/decision prerequisites, genuine-evidence retention requirements, offline operator export/current-manifest validation and hardened local JSON I/O/output handling. P15/P16 operator JSON pathname boundaries reject symbolic-link inputs and revalidate regular-file identity/content before atomic report commit; the intentional unbounded P15 raw-template reader now enforces the same non-symlink pathname rule without changing its separate resource-policy contract.
