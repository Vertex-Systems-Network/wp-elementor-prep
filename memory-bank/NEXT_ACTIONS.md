...
- P15 — **CORE FOUNDATION IN PROGRESS / CONTROLLED TARGET PROOF RETAINED**. Issue #723 / PR #724 completed Button typography metrics and merged as main `c887ab4d...` after repaired 7/7 exact-head gates. Terminal transport #725 is state-only and canonical P15 state is already settled `IDLE_READY_NEXT_P15_BATCH`.
...
- Issue #725 is terminal state-only finalization after PR #724. It is not a canonical active Issue/PR. Open and exact-head certify its transport PR; after its merge, do not create another reconciliation solely for the transport merge commit.
