...
6. next asset/reference work must keep `OBSERVED_PROOF_VALIDATED` and `TARGET_MANAGED_CONTENT_INTEGRITY_PASS` separate from an internal closure decision, then target durable target-managed media portability/decision semantics with exact evidence; do not add more responsive controls as a blanket claim;
