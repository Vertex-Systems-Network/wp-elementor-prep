...
6. after #572, keep `CONTROLLED_CROSS_TARGET_MEDIA_PORTABILITY_PASS` separate from the actual internal closure decision; next define any internal decision only as a separate exact contract if the retained exact evidence justifies it, and do not generalize this two-target controlled proof to arbitrary hosts, portable attachment IDs, broader media-library compatibility or blanket responsive closure;
...
Do not broaden semantics, responsive behavior or asset portability through heuristics merely to increase coverage. #556/#558/#560/#562 are bounded responsive slices and #564 is one bounded URL-only → target-managed media rewrite observation, #566 is only its sanitized observed-evidence binding, #568 proves only exact imported-file byte integrity, #570 only makes those exact prerequisites review-ready, and #572 proves only one exact controlled cross-target re-import path. A separate explicit internal closure decision is now the next primary concrete P15 decision gap; arbitrary-host/general media portability remains separately unclaimed.
