...
2. keep responsive decisions explicit and exact-bound: #556 writes only direction, #558 linked-px gap, #560 verified flex align/justify, #562 px padding, #576 `nowrap`/`wrap`, #578 `align_content` only after exact same-breakpoint `wrap`, and #582 uniform integer-px `border_radius`; desktop values remain preserved and omitted responsive controls remain omitted;
