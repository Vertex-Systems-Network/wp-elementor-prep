...
import {
  P15_ELEMENTOR_BUTTON_BACKGROUND_COLOR_EVIDENCE,
  P15_ELEMENTOR_BUTTON_BACKGROUND_COLOR_MANIFEST_VERSION,
  resolveP15ElementorButtonBackgroundColors,
  serializeP15ElementorButtonBackgroundColorSummary,
  type P15ElementorButtonBackgroundColorManifestV1,
  type P15ElementorButtonBackgroundColorResultV1,
} from '../src/targets/elementor/button-background-color-resolution';
import {
...
function sourceDocument(): P15NeutralExportDocumentV1 {
  return {
    ...
    title: 'Button background private source',
...
function manifest(
  ...
  buttons: P15ElementorButtonBackgroundColorManifestV1['buttons'],
): P15ElementorButtonBackgroundColorManifestV1 {
  return {
    schemaVersion: 1,
...
describe('P15 Button normal classic background color', () => {
  it('writes only background_background=classic plus background_color', () => {
    const source = sourceDocument();
    const result = resolveP15ElementorButtonBackgroundColors(source, manifest(source, [
      { sourceNodeId: 'button', color: '#1a2b3c' },
    ]));

    expect(P15_ELEMENTOR_BUTTON_BACKGROUND_COLOR_EVIDENCE).toEqual({
    ...
    expect(result.status).toBe('BUTTON_BACKGROUND_COLORS_RESOLVED');
    ...
    expect(result.resolvedBackgrounds).toEqual([{ sourceNodeId: 'button', color: '#1a2b3c' }]);
  ...
  it('is deterministic and fails closed for stale replay', () => {
    const source = sourceDocument();
    const first = resolveP15ElementorButtonBackgroundColors(source, manifest(source, []));
    const second = resolveP15ElementorButtonBackgroundColors(source, manifest(source, []));
    expect(first.status).toBe('NO_BUTTON_BACKGROUND_COLOR_OVERRIDES');
    expect(first).toEqual(second);

    const valid = manifest(source, [{ sourceNodeId: 'button', color: '#123456' }]);
    const staleSource = resolveP15ElementorButtonBackgroundColors(source, {
      ...valid,
      sourceIrFingerprint: 'sha256:' + '0'.repeat(64),
    });
    expect(staleSource.issues.map((issue) => issue.code))
      .toContain('P15_BUTTON_BACKGROUND_COLOR_SOURCE_FINGERPRINT_MISMATCH');

    const staleCandidate = resolveP15ElementorButtonBackgroundColors(source, {
      ...valid,
      baseCandidateIdentityDigest: 'sha256:' + '1'.repeat(64),
    });
    expect(staleCandidate.issues.map((issue) => issue.code))
      .toContain('P15_BUTTON_BACKGROUND_COLOR_BASE_CANDIDATE_IDENTITY_MISMATCH');
  });

  it('rejects invalid ids/colors and gradient/hover/type expansion fields', () => {
    const source = sourceDocument();

    const duplicate = resolveP15ElementorButtonBackgroundColors(source, manifest(source, [
      { sourceNodeId: 'button', color: '#112233' },
      { sourceNodeId: 'button', color: '#445566' },
    ]));
    expect(duplicate.issues.map((issue) => issue.code))
      .toContain('P15_BUTTON_BACKGROUND_COLOR_DUPLICATE_SOURCE_ID');

    const nonButton = resolveP15ElementorButtonBackgroundColors(source, manifest(source, [
      { sourceNodeId: 'heading', color: '#112233' },
    ]));
    expect(nonButton.issues.map((issue) => issue.code))
      .toContain('P15_BUTTON_BACKGROUND_COLOR_SOURCE_NOT_BUTTON');

    for (const color of ['', '#fff', '#ABCDEF', '#11223344', 'red', 'transparent', 'var(--e-global-color-accent)']) {
      const invalid = resolveP15ElementorButtonBackgroundColors(source, {
        ...manifest(source, []),
        buttons: [{ sourceNodeId: 'button', color }],
      });
      expect(invalid.issues.map((issue) => issue.code))
        .toContain('P15_BUTTON_BACKGROUND_COLOR_VALUE_INVALID');
    }
    ...
    for (const extra of [
      ...
      expect(invalid.issues.map((issue) => issue.code))
        .toContain('P15_BUTTON_BACKGROUND_COLOR_ENTRY_INVALID');
    }
  });
  ...
  it('rejects authority inflation and serializes sanitized metadata only', () => {
    ...
    const inflatedManifest = resolveP15ElementorButtonBackgroundColors(source, {
      ...raw,
      productionAcceptance: true,
    });
    expect(inflatedManifest.issues.map((issue) => issue.code))
      .toContain('P15_BUTTON_BACKGROUND_COLOR_AUTHORITY_FLAGS_INVALID');

    const result = resolveP15ElementorButtonBackgroundColors(source, raw);
    const serialized = serializeP15ElementorButtonBackgroundColorSummary(result);
    ...
    const inflatedResult = { ...result, networkAccess: true } as unknown as P15ElementorButtonBackgroundColorResultV1;
    expect(() => serializeP15ElementorButtonBackgroundColorSummary(inflatedResult))
