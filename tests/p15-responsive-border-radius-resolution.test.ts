...
import {
  P15_ELEMENTOR_RESPONSIVE_BORDER_RADIUS_EVIDENCE,
  P15_ELEMENTOR_RESPONSIVE_BORDER_RADIUS_MANIFEST_VERSION,
  resolveP15ElementorResponsiveContainerBorderRadius,
  serializeP15ElementorResponsiveBorderRadiusSummary,
  type P15ElementorResponsiveBorderRadiusManifestV1,
  type P15ElementorResponsiveBorderRadiusResultV1,
} from '../src/targets/elementor/responsive-border-radius-resolution';
import {
...
function manifest(
  ...
  containers: P15ElementorResponsiveBorderRadiusManifestV1['containers'],
): P15ElementorResponsiveBorderRadiusManifestV1 {
  return {
    schemaVersion: 1,
...
describe('P15 exact source-bound responsive container border-radius resolution', () => {
  it('writes exact Elementor 4.2.4 tablet/mobile radius controls while preserving desktop radius', () => {
    ...
    const result = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, [
      {
        sourceNodeId: 'root',
    ...
    expect(P15_ELEMENTOR_RESPONSIVE_BORDER_RADIUS_EVIDENCE).toEqual({
    ...
    expect(result.status).toBe('RESPONSIVE_BORDER_RADIUS_RESOLVED');
    ...
    expect(settings.border_radius_tablet).toEqual(expectedDimensions(20));
    expect(settings.border_radius_mobile).toEqual(expectedDimensions(6));
    expect(result.resolvedBorderRadii).toEqual([{
      sourceNodeId: 'root',
      tabletCornerRadiusPx: 20,
      mobileCornerRadiusPx: 6,
    }]);
  ...
  it('supports zero radius, binds nested containers, and omits unspecified breakpoints', () => {
    ...
    expect(rootSettings).not.toHaveProperty('border_radius_tablet');
    expect(rootSettings).not.toHaveProperty('border_radius_mobile');
    expect(nestedSettings.border_radius).toEqual(expectedDimensions(8));
    expect(nestedSettings).not.toHaveProperty('border_radius_tablet');
    expect(nestedSettings.border_radius_mobile).toEqual(expectedDimensions(0));
    expect(result.resolvedBorderRadii).toEqual([{
      sourceNodeId: 'nested',
  ...
  it('accepts the existing neutral radius ceiling and returns a deterministic no-op for no overrides', () => {
    const source = sourceDocument();
    const max = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, [
      { sourceNodeId: 'root', tabletCornerRadiusPx: P15_NEUTRAL_EXPORT_MAX_RADIUS_PX },
    ]));
    expect(settingsOf(max.template?.content[0]).border_radius_tablet)
      .toEqual(expectedDimensions(P15_NEUTRAL_EXPORT_MAX_RADIUS_PX));

    const first = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, []));
    const second = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, []));
    expect(first.status).toBe('NO_RESPONSIVE_BORDER_RADIUS_OVERRIDES');
  ...
  it('fails closed for stale source/candidate replay and invalid entries', () => {
    ...
    const staleSource = resolveP15ElementorResponsiveContainerBorderRadius(source, {
      ...valid,
      sourceIrFingerprint: 'sha256:' + '0'.repeat(64),
    });
    expect(staleSource.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_SOURCE_FINGERPRINT_MISMATCH');

    const staleCandidate = resolveP15ElementorResponsiveContainerBorderRadius(source, {
      ...valid,
      baseCandidateIdentityDigest: 'sha256:' + '1'.repeat(64),
    });
    expect(staleCandidate.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_BASE_CANDIDATE_IDENTITY_MISMATCH');

    const duplicate = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, [
    ...
    expect(duplicate.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_DUPLICATE_SOURCE_ID');

    const nonContainer = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, [
      { sourceNodeId: 'copy', mobileCornerRadiusPx: 1 },
    ]));
    expect(nonContainer.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_SOURCE_NOT_CONTAINER');

    const empty = resolveP15ElementorResponsiveContainerBorderRadius(source, manifest(source, [
      { sourceNodeId: 'root' },
    ]));
    expect(empty.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_OVERRIDE_REQUIRED');
    ...
    for (const value of [
      ...
      const invalid = resolveP15ElementorResponsiveContainerBorderRadius(source, {
        ...manifest(source, []),
        containers: [{ sourceNodeId: 'root', mobileCornerRadiusPx: value }],
      });
      expect(invalid.issues.map((issue) => issue.code))
        .toContain('P15_RESPONSIVE_BORDER_RADIUS_VALUE_INVALID');
    }

    const unknownEntryField = resolveP15ElementorResponsiveContainerBorderRadius(source, {
      ...manifest(source, []),
      containers: [{
        sourceNodeId: 'root',
    ...
    expect(unknownEntryField.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_ENTRY_INVALID');
  });
  ...
  it('blocks review-bearing upstream IR and rejects authority inflation', () => {
    ...
    const blocked = resolveP15ElementorResponsiveContainerBorderRadius(blockedSource, {});
    ...
    expect(blocked.issues).toEqual([
      expect.objectContaining({ code: 'P15_RESPONSIVE_BORDER_RADIUS_UPSTREAM_GENERATION_NOT_READY' }),
    ]);

    const source = sourceDocument();
    const raw = manifest(source, [{ sourceNodeId: 'root', mobileCornerRadiusPx: 4 }]);
    const inflated = resolveP15ElementorResponsiveContainerBorderRadius(source, {
      ...raw,
      responsiveClosureClaim: true,
    });
    expect(inflated.issues.map((issue) => issue.code))
      .toContain('P15_RESPONSIVE_BORDER_RADIUS_AUTHORITY_FLAGS_INVALID');
  });

  it('serializes only sanitized radius metadata and refuses authority-inflated results', () => {
    const source = sourceDocument();
    const raw = manifest(source, [{ sourceNodeId: 'root', mobileCornerRadiusPx: 4 }]);
    const result = resolveP15ElementorResponsiveContainerBorderRadius(source, raw);
    const serialized = serializeP15ElementorResponsiveBorderRadiusSummary(result);

    expect(serialized).not.toContain('PRIVATE RADIUS COPY');
    expect(serialized).not.toContain('templateJson');
    expect(serialized).not.toContain('"candidate"');
    expect(serialized).not.toContain('"template"');
    expect(serialized).toContain('"border_radius_mobile"');
    expect(serialized).toContain('"mobileCornerRadiusPx": 4');

    const mutatedIssue = {
      ...resolveP15ElementorResponsiveContainerBorderRadius(source, {
        ...raw,
    ...
    expect(serializeP15ElementorResponsiveBorderRadiusSummary(mutatedIssue))
      .not.toContain('PRIVATE RADIUS COPY');

    const inflatedResult = {
      ...result,
      downloadEnabled: true,
    } as unknown as P15ElementorResponsiveBorderRadiusResultV1;
    expect(() => serializeP15ElementorResponsiveBorderRadiusSummary(inflatedResult))
