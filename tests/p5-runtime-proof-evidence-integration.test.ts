...
describe('P7 embedded P5 proof/evidence integration', () => {
  it('mints proof only after deterministic acceptance', async () => {
    ...
    const accepted = await updateP5RuntimeProofFromCalibration(storage, passingResult(), TEST_BUILD);
    ...
    const rejected = await updateP5RuntimeProofFromCalibration(storage, inconsistent, TEST_BUILD);
  ...
  it('rejects invalid pixel evidence even when top-level self-test says PASS', async () => {
    ...
    const assessment = await updateP5RuntimeProofFromCalibration(storage, inconsistent, TEST_BUILD);
