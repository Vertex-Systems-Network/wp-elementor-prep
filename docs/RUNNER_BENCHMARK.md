...
| RQ-653-FINAL | #653 | P14 R4 vertical-stack retained-duplicate Figma runtime adapter | Required PR exact-head gate set | candidate-only adapter; P5 semantics + #651 address resolution + #649 validation; source immutable; separate retained duplicate; production registry remains empty | `FINAL_BATCH` | focused fake-Figma tests plus exact-head required repository checks PASS before merge | IMPLEMENTATION_PENDING_PR |
