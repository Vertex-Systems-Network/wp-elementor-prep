...
| RQ-653-FINAL | #653 / PR #654 | P14 R4 vertical-stack retained-duplicate Figma runtime adapter | Required PR exact-head gate set | first head fixed TS7022; repaired head `96f51eb41b1bae0d5a3b8985366e4ffd9e11eb1c` passed typecheck then reached 1597 PASS / 2 FAIL in the new adapter suite; `d9ce315...` now rejects hidden candidate roots as insufficient evidence and `fca1180...` aligns the source-identity refusal regression; no authority/security gate weakened | `FINAL_BATCH` | second-repaired exact-head fake-Figma tests plus required repository checks PASS before merge | REPAIRED_AWAITING_NEW_EXACT_HEAD_BATCH |
