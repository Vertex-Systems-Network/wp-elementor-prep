...
| RQ-653-FINAL | #653 / PR #654 | P14 R4 vertical-stack retained-duplicate Figma runtime adapter + README progress contract | Required PR exact-head gate set | TS7022 + two focused adapter test failures repaired; README now shows P14 67% impl (R1-R4/6), protocol requires progress sync on every material mutation, and `status:verify` machine-checks runtime-adapter/empty-registry README truth; exact head changed and needs fresh certification | `FINAL_BATCH` | README-synchronized repaired exact-head fake-Figma tests plus required repository checks PASS before merge | REPAIRED_AWAITING_NEW_EXACT_HEAD_BATCH |
