...
| RQ-653-FINAL | #653 | P14 R4 vertical-stack retained-duplicate Figma runtime adapter | Required PR exact-head gate set | concrete candidate-only adapter implemented; exact P13 fingerprint + #651 address resolution + P5 transform + #649 validation + deterministic rescore + owned retain/discard; qualification v3 leaves production registry as sole blocker | `FINAL_BATCH` | focused fake-Figma tests plus exact-head required repository checks PASS before merge | IMPLEMENTED_AWAITING_PR |
