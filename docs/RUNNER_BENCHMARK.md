...
| RQ-651-FINAL | #651 | P14 R3 vertical-stack candidate target-addressing contract | Required PR exact-head gate set | deterministic source-root-bound candidate paths; clone-ID divergence/stale-path/wrong-root/source-node protection; production registry remains empty | `FINAL_BATCH` | focused tests plus exact-head required repository checks PASS before merge | IMPLEMENTATION_PENDING_PR |
