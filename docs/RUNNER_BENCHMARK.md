...
| RQ-701-FINAL | #701 / PR #702 | P15 Button normal classic background color v1 | Required PR exact-head gate set | exact Elementor 4.2.4 source-bound write surface: `background_background=classic` + `background_color`; gradients/hover/global-token inference and authority expansion false | `FINAL_BATCH` | final bound PR head must pass CI/CodeQL/Integration/P12 Offline/P12 Final/P15 target/P17 browser before merge | PR_OPEN_AWAITING_EXACT_HEAD_BATCH |
