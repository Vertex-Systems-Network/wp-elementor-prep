...
| RQ-649-FINAL | #649 | P14 R2 vertical-stack validation-profile contract | Required PR exact-head gate set | versioned profile/check contract; fail-closed evidence coverage; qualification binding remains non-authorizing; production registry stays empty | `FINAL_BATCH` | focused tests plus exact-head required repository checks PASS before merge | IMPLEMENTATION_PENDING_PR |
