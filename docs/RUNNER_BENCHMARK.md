...
| RQ-705-FINAL | #705 / PR #706 | P15 Button hover classic background color v1 | Required PR exact-head gate set | exact Elementor 4.2.4 source-bound write surface: `button_background_hover_background=classic` + `button_background_hover_color`; hover text/normal styling/gradient expansion false | `FINAL_BATCH` | final bound PR head must pass CI/CodeQL/Integration/P12 Offline/P12 Final/P15 target/P17 browser before merge | PR_OPEN_AWAITING_EXACT_HEAD_BATCH |
