...
| RQ-651-FINAL | #651 | P14 R3 vertical-stack candidate target-addressing contract | Required PR exact-head gate set | bounded source-root/fingerprint-bound child paths + clone-stable witnesses implemented; source IDs remain non-authorizing; focused regressions added; runtime/production registry remain closed | `FINAL_BATCH` | focused tests plus exact-head required repository checks PASS before merge | IMPLEMENTED_AWAITING_PR |
