...
| RQ-703-FINAL | #703 / PR #704 | P15 Button hover text color v1 | Required PR exact-head gate set | initial head `d9eeadda...`: 5/7 PASS; CI + P12 Final failed same status-verifier placement syntax defect; verifier-only repair preserves exact `hover_color` write surface and authority boundaries | `FINAL_BATCH` | repaired final head must pass all seven required gates before merge | REPAIR_PUSHED_AWAITING_EXACT_HEAD_BATCH |
