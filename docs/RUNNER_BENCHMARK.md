...
| RQ-649-FINAL | #649 / PR #650 | P14 R2 vertical-stack validation-profile contract | Required PR exact-head gate set | prior head `cf5a98b7...` failed CI/P12 Final on the same strict TypeScript test-fixture typing issue; repair commit `2941d185...` uses explicit required fixture fields without weakening validation/authority checks | `FINAL_BATCH` | repaired exact-head focused tests plus required repository checks PASS before merge | REPAIRED_AWAITING_NEW_EXACT_HEAD_BATCH |
