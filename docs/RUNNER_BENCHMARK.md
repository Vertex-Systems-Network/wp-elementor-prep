...
| RQ-634-WINSTAT | #634 | Diagnose Node 22 Windows path-stat vs open-handle stat identity mismatch without weakening race detection | Node 22 Windows Stat Probe | synthetic temporary file only; compare dev/inode/size/time/link metadata and canonical-path equality; retain run identity, then remove probe workflow | BLOCKING_NOW |
