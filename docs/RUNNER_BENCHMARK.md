...
| RQ-703-FINAL | #703 / PR #704 | P15 Button hover text color v1 | Required PR exact-head gate set | initial head `d9eeadda...`: 5/7 PASS, verifier syntax failure; first repaired head `7cf9d1df...`: syntax fixed but stale #701 README assertion surfaced; second verifier-only repair `83d5aa41...` aligns canonical merged wording; product scope unchanged | `FINAL_BATCH` | final repaired head must pass all seven required gates before merge | REPAIR_PUSHED_AWAITING_EXACT_HEAD_BATCH |
