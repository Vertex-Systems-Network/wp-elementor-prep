...
| RQ-655-FINAL | #655 / PR #656 | P14 R5 exact vertical-stack production planning-registry binding | Required PR exact-head gate set | exact `BR_SAFE_VERTICAL_STACK_CANDIDATE@1` → `P14_VERTICAL_STACK_V1@1`; qualification v4; wrong-recipe refusal; runtime/confirmation/UI authority remains false | `FINAL_BATCH` | exact-head registry/handoff/adapter/qualification + repository checks PASS before merge | PR_OPEN_AWAITING_EXACT_HEAD_BATCH |
